...
?><C><h3><green>Releasing Notes<green></h3> </C> 
...
//Show the first 3 realising notes (News)
?><br><br><br><br><?php
$result = mysqli_query($con,"SELECT * FROM news ORDER BY day DESC");
$row = mysqli_fetch_row($result);
$n = mysqli_num_rows($result);
//emeis 8eloume mono ta 3 teleutea nea! ara an uparxoun parapanw pali mono ta 3 emfanise
if($n>3) $n=3;
for($i=0;$i<$n;$i++) 
{
	echo "<deleteNot><b>$row[1]</b><br></deleteNot><br>";
	echo nl2br($row[0]);
	
	if (isset($_SESSION['status']) && $_SESSION['status']=="admin") 
	{
		echo "<form method=\"POST\" action=\"/home.php\">";
		echo "<input type=\"Hidden\" name=\"noteId\" value='$row[2]'><br>";
