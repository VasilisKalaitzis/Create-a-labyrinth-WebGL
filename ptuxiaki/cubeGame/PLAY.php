...
function startup() {
	...
	//cheesesLeft=5;
//Instead of the above trial code, now we will fill the stage through our database
	fillTheStage();
	anim(); 
...
//Time
var d2 = new Date();
var currentTime = d2.getTime() / 1000;
if(stageCleared==0)
{
	score = Math.floor(currentTime - startTime);
	document.getElementById("timeId").innerHTML = score;
}
//
...
//The next two lines are important to bind the buffer and set the shaders
//Usually the shader and the current buffer were set from the createPointerBox() function but in this specific one, the pointerBox shouldn't be called
	gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffer);
	gl.vertexAttribPointer(shaderProgram.texturePositionAttribute, textureBuffer.itemSize, gl.FLOAT, false, 0, 0);	
...
//Now we need to sort the array, because of troubles when creating objects with lower Z than the existing ones
	...
	if(kthrio==10)
	{
		document.getElementById("cheeseNumber").innerHTML = cheesesLeft;
				...
				else if(building[i]==10)
				{
					if(kameraX-locationsX[i]>-(cubeSize/3+forwardSpeed) && kameraX-locationsX[i]<(cubeSize/3+forwardSpeed) && kameraY-locationsY[i]<(cubeSize/3+forwardSpeed) && kameraY-locationsY[i]>-(cubeSize/3+forwardSpeed) && kameraZ-locationsZ[i]<cubeSize/2 && kameraZ-locationsZ[i]>-cubeSize/2)  
					{
						...
						if(cheesesLeft==0) 
						{
							...
							stageCleared=1;
							 document.getElementsByName("b1")[0].style.display="block";
...
<script>
//Get From Database
function fillTheStage()
{
	<?php
	$id=$_GET['StageID'];
	//Get Info from database from the table: stage
	$result = mysqli_query($con,"SELECT stageLevel,UserX,UserY,UserZ,numberOfCheeses,numberOfBuildings FROM stage Where id=$id");
	$row = mysqli_fetch_row($result);
	//only 1 row can exist in the table with the specific id!
	echo "currentWall=$row[0];";
	echo "kameraX=$row[1];";
	echo "kameraY=$row[2];";
	echo "kameraZ=$row[3];";
	echo "cheesesLeft=$row[4];";
	echo "ii=$row[5];";
	//get info from database from the table: buildings
	$result = mysqli_query($con,"SELECT building,locX,locY,locZ FROM buildings Where stageID=$id");
	$row = mysqli_fetch_row($result);
	$n = mysqli_num_rows($result);
	for($i=0;$i<$n;$i++) 
	{
		echo "building[$i]=$row[0];";
		echo "locationsX[$i]=$row[1];";
		echo "locationsY[$i]=$row[2];";
		echo "locationsZ[$i]=$row[3];";
		$row = mysqli_fetch_row($result);
	}
	?>
	document.getElementById("cheeseNumber").innerHTML = cheesesLeft;
	//var dataObject = { id: '<?=$_GET['StageID']?>'};
	//
	//alert("asdasd");
	//$.ajax({type: "POST",
	//		url: "getStageFromDatabase.php",
	//	data: dataObject,
	//	cache: false
	//	});
...
<?php
//variable to send to database
if (isset($_SESSION['user'])) $scoreUser=$_SESSION['user'];
else $scoreUser="";


?>
<script>
//SEND HICHSCORE TO DATABASE!!
$(document).ready(function() {
	$('#SendHighscoreToDatabase').on('click',function() 
	{{
	
			if('<?=$scoreUser?>' == "") 
			{
				alert("You must log in to submit your highscore");
			}
			else
			{
							//Insert Score into Highscore table
							var dataObject = { stageID: '<?=$_GET['StageID']?>',
												user: '<?=$scoreUser?>',
												score : score
												};
						
							$.ajax({type: "POST",
								url: "insertScoreToDatabase.php",
								data: dataObject,
								cache: false
								});
								
							//wait some seconds to redict on start
							setTimeout(
								function() 
								{
									window.location.href='/cubeGame.php';
								}, 100);
			}
...
<cheeseIcon><img src="cheesesLeft.png" style="height:6em; width:6em;">  
</cheeseIcon>
<cheeseIcon2>
	<div id="cheeseNumber" style="height:1em; width:1em" class="CheeseNumber">
    0
	</div>
</cheeseIcon2>

<timeIcon><img src="clock.png" style="height:8em; width:8em;">  
</timeIcon>
<timeIcon2>
	<div id="timeId" style="height:1em; width:1em" class="timeId">
    0
	</div>
</timeIcon2>

<GameCanvas3>
 <button type=Submit id="SendHighscoreToDatabase" name="b1" value="" onmouseover="" style="display: none; cursor: pointer; color:green; height: 4em; width: 30em; background: url('/cubeGame/save.png') no-repeat scroll 0 0 transparent; border: none;"></button>
</GameCanvas3>
