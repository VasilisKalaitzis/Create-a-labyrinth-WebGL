...
<pic2>    <img src="creationStart.jpg" style="height:20em; width:40em;">         </pic2>
<StartCreation>
<?php
//an prohgoumenos epele3e god's mode h builder's mode
if(isset($_GET['mode'])) 
{
	if($_GET['mode']=="God")
	{ 
		?> 
		<form method=POST action=/cubeGame/GodMode.php> 
		&nbsp&nbsp&nbsp&nbsp<bigGreen><b>Stage's Name</b><br><br>
		<input type=text2 size=18 name=stageName style="font-size: 70%;"></bigGreen>
		<br> 
		<br><br><br><br>
		...
		<white_bold2>
		<input type=Submit id="button2" name=Start value="" onmouseover="" style="cursor: pointer; color:green;">
		</white_bold2>
		</Form>
		<?php
	}
	...
	{ 
		?> 
		<form method=POST action=/cubeGame/BuilderMode.php> 
		&nbsp&nbsp&nbsp&nbsp<bigGreen><b>Stage's Name</b><br><br>
		<input type=text2 size=18 name=stageName style="font-size: 70%;"></bigGreen>
		<br>
		<br><br><br><br>
...
$('input[type="text2"]').on('keyup',function() {
    var text_value = $('input[type="text2"]').val();
	var text_length = text_value.length;
    if((text_value != '' && text_value != '') && text_length<=6) {
        $('input[type="submit"]').attr('disabled' , false);
		$('input[name="Start"]').css("background", "url('../playNow.png') no-repeat");
    }else{
        $('input[type="submit"]').attr('disabled' , true);
		$('input[name="Start"]').css("background", "url('../playNowDiss.png') no-repeat");
    }
});
});
</script>
